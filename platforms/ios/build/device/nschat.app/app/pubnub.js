(function() {
    // For storing items in the client
    ...
    function PN_API(setup) {
      var SUB_WINDOWING = +setup['windowing'] || DEF_WINDOWING,
        ...
        NO_WAIT_FOR_PENDING = setup['no_wait_for_pending'] || 1,
      ...
      function publish(next) {
        ...
        if (NO_WAIT_FOR_PENDING) {
          if (!PUB_QUEUE.length) {
            // console.log("we aren't waiting, shouldn't be anything in the queue");
            return;
          }
        } else {
      ...
      function _invoke_callback(response, callback, err) {
        console.log("invoke callback called");
        if (typeof response == 'object') {
          if (response['error']) {
            var callback_data = {};

            if (response['message']) {
              console.log("response message isn't null??")
      ...
      function CR(args, callback, url1, data) {
        ...
        xdr({
          ...
          success: function(response) {
            console.log("in CR successful xdr call, soon to call invoke call back");
        ...
        */
        'history': function(args, callback) {
          ...
          xdr({
            ...
            success: function(response) {
              if (typeof response == 'object' && response['error']) {
                console.log("XDR RESPONSE: " + response);
            ...
            each((channel.join ? channel.join(',') : '' + channel).split(','),
              function(channel) {
                ...
                CHANNELS[SUB_CHANNEL = channel] = {
                  ...
                  subscribed: 1,
                  callback:  console.log('some callback'), //SUB_CALLBACK = callback,
                  'cipher_key': args['cipher_key'],
                  connect: connect,
                ...
                // Presence Enabled?
                if (!presence) return;
                if (presence) console.log(presence);
                ...
                console.log("settings.subscribed");
                return;
                }
    ...
    (function() {
        ...
        var db = (function() {
          return {
            get: function(key) {
              localSettings.getString('' + key, 'NOT FOUND');
            },
